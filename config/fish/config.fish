...
function del
    ...
    # 1. Собираем список того, что будет удалено
    set -l targets (find ~/.config ~/.cache ~/.local/share -iname "*$word*" 2>/dev/null)

    if test (count $targets) -eq 0
        echo "🔍 Ничего не найдено по '$word'"
        return 0
    end

    # 2. Показываем список
    echo "🗑️  Будет удалено (count $targets):"
    for t in $targets
        echo "   $t"
    end

    # 3. Спрашиваем подтверждение
    read -l -P "Удалить всё это? [y/N] " ans
    if test "$ans" != y
        echo "❌ Отменено"
        return 1
    end
...
function waybar-restart
    pkill waybar; and ~/.config/waybar/launch.sh
end
